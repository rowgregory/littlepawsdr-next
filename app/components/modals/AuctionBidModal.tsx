...
export default function AuctionBidModal() {
  ...
  useEffect(() => {
    if (auctionBidModal) {
      setCurrentBid(Number(auctionItem?.currentBid ?? 0))
      setMinimumBid(Number(auctionItem?.minimumBid ?? auctionItem?.startingPrice ?? 0))
    }
  }, [auctionBidModal, auctionItem?.currentBid, auctionItem?.minimumBid, auctionItem?.startingPrice])
        ...
        <div className="px-5 430:px-6 pb-6">
            ...
            <div role="tabpanel" aria-label="Quick bid options">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="font-changa text-sm uppercase tracking-wide leading-none mb-0.5 text-zinc-950 dark:text-text-dark">Custom Amount</p>
          ...
          {mode === 'custom' && (
            <div role="tabpanel" aria-label="Custom bid amount">
              <label htmlFor="bid-amount" className="block font-changa text-f10 uppercase tracking-[0.25em] text-zinc-500 dark:text-muted-dark mb-2">
                Bid Amount
              </label>
              <div className="relative mb-1">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 font-changa text-sm text-zinc-500 dark:text-muted-dark" aria-hidden="true">
                  $
                </span>
                <input
                  ref={inputRef}

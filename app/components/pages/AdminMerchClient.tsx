...
import { setOpenProductDrawer } from 'app/lib/store/slices/uiSlice'
import { setInputs } from 'app/lib/store/slices/formSlice'

export function AdminMerchClient({ products }) {
  return (
    <>
      <AdminPageHeader label="Admin" title="Products" description="Manage physical products available in the LPDR store" />
        ...
        <div className="border border-border-light dark:border-border-dark overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark flex flex-col xs:flex-row items-start xs:items-center justify-between gap-3">
            <p className="text-[10px] font-mono tracking-[0.2em] uppercase text-muted-light dark:text-muted-dark shrink-0">
              All Products
              <span className="ml-2 text-primary-light dark:text-primary-dark">{products?.length}</span>
            </p>

            <motion.button
              onClick={() => store.dispatch(setOpenProductDrawer())}
            ...
            <table className="w-full" role="table" aria-label="Product list">
              <thead>
                <tr className="border-b border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark">
                  {['Name', 'Price', 'Shipping', 'Quantity', 'Sold', ''].map((h) => (
                    <th
                      key={h}
                ...
                {products?.length === 0 ? (
                  ...
                  products?.map((product, i) => (
                    ...
                    >
                      ...
                      {/* Price */}
                      <td className="px-5 py-4">
                        <span className="text-xs font-mono text-muted-light dark:text-muted-dark">{product.price}</span>
                      </td>

                      {/* Shipping Price */}
                      <td className="px-5 py-4">
                        <span className="text-xs font-mono text-muted-light dark:text-muted-dark">{product.shippingPrice ?? 0}</span>
                      </td>
                      ...
                      {/* Sold */}
                      <td className="px-5 py-4">
                        <span className="text-xs font-mono text-muted-light dark:text-muted-dark">{product.sold}</span>
                      </td>
                      ...
                      <td className="px-5 py-4 text-right">
                        <button
                          onClick={() => {
                            store.dispatch(setInputs({ formName: 'productForm', data: { ...product, isUpdating: true } }))
                            store.dispatch(setOpenProductDrawer())
                          }}

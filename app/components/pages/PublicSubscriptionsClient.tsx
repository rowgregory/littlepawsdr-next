  ...
  return (
    ...
    `}
      ...
      >
        ...
        <span
          className={`font-bebas font-bold text-[10px] sm:text-3xl text-center leading-tight px-2 pointer-events-none transition-colors duration-200 ${name_c}`}
        >
          {tier.name}
        </span>
      </motion.button>
...
function SubscriptionPayment({ setView, isDark, selectedTier, billing, savedPaymentMethods }) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] min-h-screen">
        ...
        <div className="px-6 sm:px-10 pt-12 sm:pt-16 pb-24">
          <div className="max-w-md w-full mx-auto lg:mx-0">
            <StepIndicator current={isAuthed ? 3 : 2} total={3} labels={paymentStepLabels} />
            <SignedInRow />

            <div className="mt-8">
              <SubscriptionPaymentForm tier={selectedTier!} billing={billing} savedCards={savedPaymentMethods} />
            </div>
          </div>
        </div>
      </div>
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function PublicSubscriptionsClient({ savedPaymentMethods }: { savedPaymentMethods: IPaymentMethod[] }) {
  // ── Store ────────────────────────────────────────────────────────────────

  const [billing, setBilling] = useState<'MONTHLY' | 'YEARLY'>('MONTHLY')
  const [view, setView] = useState<'select' | 'payment'>('select')

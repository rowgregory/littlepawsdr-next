              ...
              <li key={sub.id} className={`bg-bg-light dark:bg-bg-dark p-5 ${isCancelled ? 'opacity-60' : ''}`}>
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="min-w-0">
                    <p className="font-quicksand font-black text-base text-text-light dark:text-text-dark leading-snug truncate">{sub.tierName}</p>
                    {isCancelled && (
                ...
                {sub.nextBillingDate && sub.status === 'CONFIRMED' && (
                  <p className="text-[10px] font-mono text-muted-light dark:text-muted-dark mt-2">Next billing {formatDate(sub.nextBillingDate)}</p>
                )}

                {sub.nextBillingDate && isCancelled && (
                  <p className="text-[10px] font-mono text-red-500/70 dark:text-red-400/70 mt-2">Active until {formatDate(sub.nextBillingDate)}</p>
                )}

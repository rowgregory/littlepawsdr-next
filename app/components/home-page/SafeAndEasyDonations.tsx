...
const SafeAndEasyDonations = () => {
  const welcomeWiener = useAppSelector(
    (state: RootState) => state.welcomeWiener
  );
  ...
  return (
    <div className="max-w-[1150px] w-full mx-auto mb-40">
      <p className="text-teal-400 text-xl font-semibold mb-5">
        Safe & Easy Donations
      </p>
      <div className="flex items-center justify-between mb-10">
        ...
        <Link
          href="/dachshunds"
          className="bg-teal-400 text-white py-4 px-9 rounded-lg w-fit"
        >
          View All
        </Link>
      </div>
      <div className="grid grid-cols-12 gap-10">
        {welcomeWieners?.map((obj: any, i: number) => (
          <WelcomeWienerCard key={i} obj={obj} />
        ))}
      </div>
    </div>
  );

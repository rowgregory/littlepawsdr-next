...
export function BidsTable({ bids }: { bids: IAuctionBid[] }) {
  ...
  return (
    <section aria-labelledby="bid-history" className="border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark">
      <div className="px-4 py-2.5 border-b border-border-light dark:border-border-dark">
        <h3 id="bid-history" className="text-[9px] font-mono tracking-[0.2em] uppercase text-muted-light dark:text-muted-dark">
          ...
          >
            {sorted.length > 0 ? (
              sorted.map((bid, i) => {
                ...
                const isTop = i === 0
                return (
                  <tr key={bid.id} className={isTop ? 'bg-emerald-500/5' : 'hover:bg-primary-light/5 dark:hover:bg-primary-dark/5 transition-colors'}>
                    ...
                    <td className="px-4 py-2.5 min-w-0">
                      <p className="text-xs font-mono text-text-light dark:text-text-dark truncate max-w-40 sm:max-w-none">{name}</p>
                      <p className="sm:hidden text-[9px] font-mono text-muted-light dark:text-muted-dark mt-0.5">{formatDateTime(bid.createdAt)}</p>
                    </td>

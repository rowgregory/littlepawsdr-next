...
import { manifyingGlassIcon } from 'app/lib/font-awesome/icons'
...
const SearchBar = ({ setText }: { setText: (text: string) => void }) => {
  return (
    <div className="w-full max-w-md flex items-center border-[1px] border-gray-200 rounded-2xl bg-white h-[34px] px-2">
      ...
      <input onChange={(e: any) => setText(e.target.value)} className="w-full h-full focus:outline-0 rounded-lg ml-2" placeholder="Search" />
    </div>
  )
}

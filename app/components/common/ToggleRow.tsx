export function ToggleRow({ checked, onToggle, label, hint }: { checked: boolean; onToggle: () => void; label: string; hint: string }) {
  return (
    <button
      type="button"
      role="switch"
    ...
    >
      <div className="text-left">
        ...
        <p className="text-[10px] font-mono text-muted-light dark:text-muted-dark mt-0.5">{hint}</p>
      </div>
      <div
        aria-hidden="true"
      ...
      >
        <span
          className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white transition-transform duration-200 ${checked ? 'translate-x-5' : 'translate-x-0'}`}
        />
      </div>
    </button>
  )

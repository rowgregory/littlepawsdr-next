...
import { fileAltIcon, listIcon } from 'app/lib/font-awesome/icons'
...
const usePaginationControls = (pageAmounts: number) => {
  ...
  const PageLimitSelect = () => (
    <div className="flex items-center gap-2 px-4 py-2 border-[1px] border-gray-200 rounded-2xl">
      ...
      <select value={pageLimit} onChange={handlePageLimitChange} className="focus:outline-none cursor-pointer">
  ...
  const PageSelect = () => (
    <div className="flex items-center gap-2 px-4 py-2 border-[1px] border-gray-200 rounded-2xl">
      ...
      <select value={currentPage} onChange={handlePageChange} className="focus:outline-none cursor-pointer">
        {Array.from({ length: pageAmounts }, (_, index) => index + 1).map((page) => (

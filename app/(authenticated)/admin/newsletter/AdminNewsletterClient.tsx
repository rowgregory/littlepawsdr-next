...
// ─── Subscribers ──────────────────────────────────────────────────────────────
function SubscribersPanel({ newsletters }: { newsletters: Newsletter[] }) {
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      ...
      {totalPages > 1 && <Pagination page={safePage} totalPages={totalPages} onPage={setPage} totalItems={filtered.length} pageSize={PAGE_SIZE} />}
    </div>
  )
}

// ─── Issues ───────────────────────────────────────────────────────────────────
function IssuesPanel({ issues }: { issues: NewsletterIssue[] }) {
  const columns: Column<NewsletterIssue>[] = [
    {
      ...
      cell: (issue) => (
        <div className="min-w-0">
          <p className="text-xs font-mono font-medium text-text-light dark:text-text-dark truncate">{issue.title}</p>
          {issue.description && <p className="text-[10px] font-mono text-muted-light dark:text-muted-dark mt-0.5 truncate">{issue.description}</p>}
        </div>
      )
    },
    {
      header: 'Published',
      className: 'text-[10px] font-mono tracking-widest text-muted-light dark:text-muted-dark whitespace-nowrap',
      cell: (issue) => formatDate(new Date(issue.publishedAt))
    },
    {
      header: 'PDF',
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      ...
      <AdminTable columns={columns} rows={issues} rowKey={(issue) => issue.id} caption="Newsletter issues" emptyMessage="No issues published yet" />
    </div>
  )
}

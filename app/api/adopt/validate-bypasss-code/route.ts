...
export async function POST(req: NextRequest) {
  try {
    ...
    if (bypassCode && process.env.BYPASS_CODE && bypassCode === process.env.BYPASS_CODE) {
      ...
      response.cookies.set('adoptFeeToken', token, {
        httpOnly: true,
      ...
      return response
    } else if (bypassCode && process.env.BYPASS_CODE && bypassCode !== process.env.BYPASS_CODE) {

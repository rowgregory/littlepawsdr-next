export const getPicturesAndVideos = (response: any) => {
  ...
  const pictureUrls = response.included?.filter((item: any) => item.type === 'pictures')?.map((item: any) => item.attributes.original.url)
  ...
  response.data.forEach((item: any) => {
    item.attributes.photos = pictures.filter((p: any) => p.objId === item.id).map((p: any) => p.url)

    item.attributes.videos = videos.filter((v: any) => v.objId === item.id).map((v: any) => v.video)
  })
